...
VIEWS = ('all', 'ElevatedActivity', 'NormalActivity')
...
def build_minute(root: Path) -> pd.DataFrame:
    reg = load_module(root / 'docs/research/continuous_vol_regime_v1/run_continuous_vol_regime.py', 'v11_reg')
    orig = load_module(root / 'docs/research/state_conditioned_frequency_v2/code/run_physical_scale_strategy.py', 'v11_orig')
    fg = load_module(root / 'docs/research/state_conditioned_frequency_v2/code/fast_grid.py', 'v11_fg')
    ...
    if ACTIVITY_FIELD not in native.columns:
        raise KeyError(ACTIVITY_FIELD)
    ...
    activity = native[['trading_day', 'ts', ACTIVITY_FIELD]].copy()
    activity = activity.rename(columns={'ts': 'timestamp', ACTIVITY_FIELD: 'activity'})
    activity['activity'] = pd.to_numeric(activity.activity, errors='coerce')
    minute = minute.merge(
        activity,
        on=['trading_day', 'timestamp'],
        how='left',
        sort=False,
        validate='one_to_one',
    )
...
def build_events(minute: pd.DataFrame) -> tuple[pd.DataFrame, int]:
    ...
    for session, z0 in minute.groupby('session', sort=False):
        ...
        activity = z.activity.to_numpy(float)
        ...
        for i in range(1, len(z)):
            if state[i] != 'HighVol' or state[i - 1] != 'NormalVol':
                continue
            raw_onsets += 1
            if i < 34:
                continue
            recent_ret = r[i - 4:i + 1]
            recent_act = activity[i - 4:i + 1]
            prior_act = activity[i - 34:i - 4]
            if (
                len(recent_ret) != 5 or len(recent_act) != 5 or len(prior_act) != 30
                or not np.isfinite(recent_ret).all()
                or not np.isfinite(recent_act).all()
                or not np.isfinite(prior_act).all()
            ):
                continue
            prior_mean = float(prior_act.mean())
            if prior_mean <= 0:
                continue
            ratio = float(recent_act.mean() / prior_mean)
            net5 = float(recent_ret.sum())
            ...
            en = i + 1
            ex = en + 3
            if en < next_allowed_entry:
                continue
            if ex >= len(z) or not valid[en:ex + 1].all():
                continue
            if not (
                np.isfinite(opened[en]) and np.isfinite(opened[ex])
                and opened[en] > 0 and opened[ex] > 0
            ):
                continue

            raw = float(np.log(opened[ex] / opened[en]) * 1e4)
            activity_state = 'ElevatedActivity' if ratio >= ACTIVITY_THRESHOLD else 'NormalActivity'
            rows.append({
                ...
                'recent5_direction': 'up' if sign > 0 else 'down',
                'net5_bp': net5,
                'future_raw_3m_bp': raw,
                'continuation_3m_bp': float(sign * raw),
                'reversal_3m_bp': float(-sign * raw),
...
def stats(z: pd.DataFrame) -> dict:
    ...
    ratio = pd.to_numeric(z.activity_ratio, errors='coerce').dropna()
    n = len(c)
    mc = float(c.mean()) if n else np.nan
    mr = float(r.mean()) if n else np.nan
    return {
        'n': int(n),
        'mean_activity_ratio': float(ratio.mean()) if len(ratio) else np.nan,
...
def run(root: Path, out: Path):
    minute = build_minute(root)
    events, raw_onsets = build_events(minute)
    rows = []
    promising = []

    for view in VIEWS:
        z = events if view == 'all' else events[events.activity_state == view]
        annual = []
        for year in YEARS:
        ...
        pooled = stats(z)
        rows.append({'view': view, 'year': 'pooled', **pooled})
    ...
    summary = {
        'schema': 'star50_activity_liquidity_dev_v11',
        ...
        'views': list(VIEWS),
        'raw_onset_count': int(raw_onsets),
        'event_count': int(len(events)),
        'activity_state_counts': {str(k): int(v) for k, v in events.activity_state.value_counts().to_dict().items()},
