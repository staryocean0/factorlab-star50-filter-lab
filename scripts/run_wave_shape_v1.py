...
def annual(year):
    ...
    for k, row in enumerate(examples):
        ...
        booked = frame.iloc[a + 2:c + 2]
        axes[1, k].plot(np.arange(1, len(q)), np.cumsum(booked.pnl_log.to_numpy()) * 10000)
        ax = axes[1, k].twinx()
        ax.step(np.arange(1, len(q)), booked.exec_pos.to_numpy(), color='grey', alpha=.35)
        ax.set_ylim(-1.3, 1.3)
