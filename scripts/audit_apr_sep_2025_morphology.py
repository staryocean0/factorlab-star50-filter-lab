...
def main():
    ...
    original_first = float(pd.read_parquet(marker, filters=[('trading_day', '==', '2020-07-23')],
    ...
    for col, ex in enumerate(examples):
        ...
        for k, candle in enumerate(candles.itertuples()):
            o, hi, lo, c = 10000 * np.log(np.array([candle.open, candle.high, candle.low, candle.close], dtype=float) / ref)
