...
SOURCE_MAIN = "f182176c18818c8dd370b5490e411a78f563919f"
...
def load_parent():
    ...
    if git_blob_bytes(PARENT_PATH.read_bytes()) != PARENT_BLOB:
        raise RuntimeError("cross-index parent runner blob drift")
    spec = importlib.util.spec_from_file_location("historical_shock_burden_parent", PARENT_PATH)
...
def attach_memory(p: pd.DataFrame) -> pd.DataFrame:
    """Attach strictly-prior 12-valid-bar shock and high-vol burden summaries."""
    q = p.copy()
    for c in ("shock_count12", "shock_excess12", "highvol_count12", "highvol_excess12"):
        q[c] = np.nan
    q["memory_available"] = False

    for _, g in q.groupby("symbol", sort=False):
        shock_history: list[float] = []
        vol_history: list[float] = []
        for i in g.index:
            if len(shock_history) >= MEMORY_BARS:
                s = np.asarray(shock_history[-MEMORY_BARS:], dtype=float)
                v = np.asarray(vol_history[-MEMORY_BARS:], dtype=float)
                q.at[i, "shock_count12"] = float(np.sum(s >= SHOCK_SIGMA))
                q.at[i, "shock_excess12"] = float(np.maximum(s - SHOCK_SIGMA, 0.0).sum())
                q.at[i, "highvol_count12"] = float(np.sum(v >= HIGHVOL_RATIO))
                q.at[i, "highvol_excess12"] = float(np.maximum(v - HIGHVOL_RATIO, 0.0).sum())
                q.at[i, "memory_available"] = True

            shock = q.at[i, "final_shock_intensity"]
            vol = q.at[i, "final_vol_ratio"]
            if pd.notna(shock) and pd.notna(vol) and np.isfinite(float(shock)) and np.isfinite(float(vol)):
                shock_history.append(float(shock))
...
def build_table(root: Path, years_5m: tuple[int, ...], years_3s: tuple[int, ...]) -> tuple[pd.DataFrame, dict]:
    raw = parent.base.load_5m(root, years_5m)
    hist = parent.attach_lag_std12(parent.base.attach_history(raw))
    hist = attach_memory(hist)
    snapshots = parent.build_e15_snapshots(root, hist, years_3s)
    feat = parent.attach_current_coordinates(hist, snapshots)
    labelled = parent.base.attach_labels(feat)
...
def design(q: pd.DataFrame, model: str) -> tuple[np.ndarray, list[str]]:
    ...
    x0, names0 = parent.base.design(q, "C")
    if model == "C":
        return x0, names0

    if model == "S":
        count = q.shock_count12.to_numpy(float)
        excess = q.shock_excess12.to_numpy(float)
    else:
        count = q.highvol_count12.to_numpy(float)
        excess = q.highvol_excess12.to_numpy(float)
    ...
    if not np.isfinite(xx).all():
        raise ValueError("nonfinite historical-memory design")
...
def cohort(frame: pd.DataFrame, years: tuple[int, ...], h: int) -> tuple[pd.DataFrame, dict]:
    ...
    final = base_future & frame.memory_available.fillna(False)
    ...
    counts = {
        ...
        "memory_available": int((base_future & frame.memory_available.fillna(False)).sum()),
        "final": int(final.sum()),
...
def compare(q: pd.DataFrame, endpoint: str, base_model: str, probes: dict, forward_gain: float,
    ...
    ps = predict(q, "S", probes[f"{h}|{endpoint}|S"], endpoint)
    ...
    for days in (5, 20):
        ...
        blocks.to_csv(out / f"blocks_{h}_{endpoint}_S_vs_{base_model}_{days}d.csv", index=False, float_format="%.17g")
...
def feature_identity_checks(frame: pd.DataFrame) -> dict:
    z = frame[frame.base_available & frame.memory_available].head(200).copy()
    if z.empty:
        raise RuntimeError("no memory feature rows")
    c, cn = design(z, "C")
    s, sn = design(z, "S")
    h, hn = design(z, "H")
    if sn != hn or s.shape != h.shape or s.shape[1] - c.shape[1] != 20:
        raise RuntimeError("S/H complexity-match invariant failed")
    if sn[:len(cn)] != cn or len(sn) != len(cn) + 20:
        raise RuntimeError("historical-memory schema invariant failed")
    return {
        "base_columns": int(c.shape[1]),
        "shock_memory_columns": int(s.shape[1]),
        "highvol_control_columns": int(h.shape[1]),
        "S_H_schema_identical": True,
        "memory_block_columns": 20,
    }
...
def forward_and_fit(frame: pd.DataFrame, out: Path, identities: dict) -> None:
    ...
    for h in HORIZONS:
        ...
        for endpoint in ENDPOINTS:
            ...
            ps = predict(hold, "S", fwd["S"], endpoint)
            for b in ("C", "H"):
    ...
    payload = {
        "schema": "historical_shock_burden_utility_v1_frozen_models",
        "source_main": SOURCE_MAIN,
        "protocol_git_blob": PROTOCOL_BLOB,
        "parent_runner_git_blob": PARENT_BLOB,
        "ridge_lambda": LAMBDA,
        "memory_bars": MEMORY_BARS,
        "shock_threshold": SHOCK_SIGMA,
        "highvol_threshold": HIGHVOL_RATIO,
        "memory_block_columns": 20,
        "models": probes,
...
def descriptive_outputs(q: pd.DataFrame, probes: dict, h: int, out: Path) -> dict:
    corr_cols = [
        "shock_intensity", "vol_ratio", "shock_count12", "shock_excess12",
        "highvol_count12", "highvol_excess12",
    ]
    corr = q[corr_cols].corr().to_dict()
    age = q.observation_age_seconds.dropna().to_numpy(float)
    age_summary = {
        "n": int(len(age)),
        "median": float(np.median(age)),
        "p95": float(np.quantile(age, 0.95)),
        "max": float(np.max(age)),
    } if len(age) else {"n": 0}
    shock_count = q.shock_count12.to_numpy(float)
    frequency = {
        "zero": int(np.sum(shock_count == 0)),
        "one": int(np.sum(shock_count == 1)),
        "multiple": int(np.sum(shock_count >= 2)),
    }
    ...
    for endpoint in ENDPOINTS:
        ...
        ps = predict(q, "S", probes[f"{h}|{endpoint}|S"], endpoint)
        ph = predict(q, "H", probes[f"{h}|{endpoint}|H"], endpoint)
        y = q[endpoint].to_numpy(float)
        for b, pb in (("C", pc), ("H", ph)):
            gain = (y - pb) ** 2 - (y - ps) ** 2
            for col in ("symbol", "year", "previous_state", "slot"):
                for key, inds in q.groupby(col, sort=True).indices.items():
                    slice_rows.append({
                        "horizon": h, "endpoint": endpoint, "comparison": f"S_vs_{b}",
        ...
        tmp = pd.DataFrame({
            "decile": dec,
            "sigma": q.sigma.to_numpy(float),
            "tail": q.future_tail.to_numpy(float),
            "increment": increment,
        })
    ...
    return {
        "correlations": corr,
        "observation_age": age_summary,
        "shock_count_frequency": frequency,
        "memory_summary": {
            c: {
                "mean": float(q[c].mean()),
...
def validate(frame: pd.DataFrame, models_path: Path, model_sha_path: Path, out: Path, identities: dict) -> None:
        ...
        for endpoint in ENDPOINTS:
            for b in ("C", "H"):
                ...
                comparisons.append(compare(
                    ...
                    frozen["models"][f"{h}|{endpoint}|S"]["n"], cov["coverage"], h, out,
                ))
    ...
    decision = (
        "HISTORICAL_SHOCK_BURDEN_INCREMENTAL_UTILITY_SUPPORTED_FOR_SPECIFIED_ENDPOINTS"
        if any(joint.values()) else "HISTORICAL_SHOCK_BURDEN_INCREMENTAL_UTILITY_NOT_SUPPORTED"
    )
    pd.DataFrame([
    ...
    dump_json(out / "VALIDATION_RESULTS.json", {
        "schema": "historical_shock_burden_utility_v1_validation",
        "decision": decision,
...
def run(args) -> None:
    ...
    if git_blob_bytes(protocol_path.read_bytes()) != PROTOCOL_BLOB:
