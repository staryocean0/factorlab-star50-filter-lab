...
SOURCE_MAIN = "e29c44cd61cdfcead7321a036850d7cd1b33006b"
...
def load_parent():
    ...
    spec = importlib.util.spec_from_file_location("degree_trajectory_parent", PARENT_PATH)
...
def attach_lag2(p: pd.DataFrame) -> pd.DataFrame:
    """Attach the two-valid-return-bars-back degree using current confirmed bg48."""
    q = p.copy()
    q["lag2_abs"] = np.nan
    q["lag2_std12"] = np.nan
    for _, g in q.groupby("symbol", sort=False):
        v = g.r.dropna()
        q.loc[v.index, "lag2_abs"] = v.shift(2).abs()
        q.loc[v.index, "lag2_std12"] = v.shift(2).rolling(12, min_periods=12).std(ddof=0)
    q["lag2_intensity"] = q.lag2_abs / q.bg48
    q["lag2_ratio"] = q.lag2_std12 / q.bg48
    q["lag2_available"] = np.isfinite(q.lag2_intensity) & np.isfinite(q.lag2_ratio)
    return q


def attach_deltas(q: pd.DataFrame) -> pd.DataFrame:
    z = q.copy()
    z["delta1_intensity"] = z.shock_intensity - z.lag_intensity
    z["delta1_ratio"] = z.vol_ratio - z.lag_ratio
    z["delta2_intensity"] = z.shock_intensity - z.lag2_intensity
    z["delta2_ratio"] = z.vol_ratio - z.lag2_ratio
    return z
...
def build_table(root: Path, years_5m: tuple[int, ...], years_3s: tuple[int, ...]) -> tuple[pd.DataFrame, dict]:
    ...
    hist = attach_lag2(hist)
    snapshots = parent.build_e15_snapshots(root, hist, years_3s)
    feat = attach_deltas(parent.attach_current_coordinates(hist, snapshots))
...
def design(q: pd.DataFrame, model: str) -> tuple[np.ndarray, list[str]]:
    ...
    if model == "T":
        i = q.lag_intensity.to_numpy(float)
        v0 = q.lag_ratio.to_numpy(float)
    else:
        i = q.lag2_intensity.to_numpy(float)
        v0 = q.lag2_ratio.to_numpy(float)

    u = np.log1p(i)
    v = np.log(np.maximum(v0, 1e-12))
    terms = {
        "trajectory_intensity": u,
        "trajectory_ratio": v,
        "trajectory_intensity2": u * u,
        "trajectory_ratio2": v * v,
        "trajectory_cross": u * v,
    }
    ...
    if not np.isfinite(xx).all():
        raise ValueError("nonfinite degree-trajectory design")
...
def cohort(frame: pd.DataFrame, years: tuple[int, ...], h: int) -> tuple[pd.DataFrame, dict]:
    ...
    trajectory_available = frame.lag_available.fillna(False) & frame.lag2_available.fillna(False)
    final = base_future & trajectory_available
    ...
    counts = {
        ...
        "lag1_available": int((base_future & frame.lag_available.fillna(False)).sum()),
        "lag2_available": int((base_future & frame.lag2_available.fillna(False)).sum()),
        "final": int(final.sum()),
...
def feature_identity_checks(frame: pd.DataFrame) -> dict:
    z = frame[frame.base_available & frame.lag_available & frame.lag2_available].head(200).copy()
    if z.empty:
        raise RuntimeError("no degree-trajectory feature rows")
    c, cn = design(z, "C")
    t, tn = design(z, "T")
    o, on = design(z, "O")
    if tn != on or t.shape != o.shape or t.shape[1] - c.shape[1] != 20:
        raise RuntimeError("T/O complexity-match invariant failed")
    if tn[:len(cn)] != cn or len(tn) != len(cn) + 20:
        raise RuntimeError("degree-trajectory schema invariant failed")
    if c.shape[1] != 84 or t.shape[1] != 104:
        raise RuntimeError(f"unexpected inherited feature counts: C={c.shape[1]} T={t.shape[1]}")

    np.testing.assert_allclose(
        z.shock_intensity.to_numpy(float) - z.delta1_intensity.to_numpy(float),
        z.lag_intensity.to_numpy(float), rtol=0, atol=1e-15,
    )
    np.testing.assert_allclose(
        z.vol_ratio.to_numpy(float) - z.delta1_ratio.to_numpy(float),
        z.lag_ratio.to_numpy(float), rtol=0, atol=1e-15,
    )
    return {
        "base_columns": int(c.shape[1]),
        "trajectory_columns": int(t.shape[1]),
        "lag2_control_columns": int(o.shape[1]),
        "T_O_schema_identical": True,
        "trajectory_block_columns": 20,
        "lag1_delta_raw_information_bijective": True,
    }
...
def forward_and_fit(frame: pd.DataFrame, out: Path, identities: dict) -> None:
    ...
    payload = {
        "schema": "degree_trajectory_utility_v1_frozen_models",
...
def descriptive_outputs(q: pd.DataFrame, probes: dict, h: int, out: Path) -> dict:
    corr_cols = [
        ...
        "lag2_intensity", "lag2_ratio", "delta1_intensity", "delta1_ratio",
    ]
...
def validate(frame: pd.DataFrame, models_path: Path, model_sha_path: Path, out: Path, identities: dict) -> None:
        ...
        for endpoint in ENDPOINTS:
            for b in ("C", "O"):
                ...
                comparisons.append(compare(
                    ...
                    frozen["models"][f"{h}|{endpoint}|T"]["n"], cov["coverage"], h, out,
                ))
    ...
    decision = (
        "ONE_STEP_DEGREE_TRAJECTORY_INCREMENTAL_UTILITY_SUPPORTED_FOR_SPECIFIED_ENDPOINTS"
        if any(joint.values()) else "ONE_STEP_DEGREE_TRAJECTORY_INCREMENTAL_UTILITY_NOT_SUPPORTED"
    )
    pd.DataFrame([
    ...
    dump_json(out / "VALIDATION_RESULTS.json", {
        "schema": "degree_trajectory_utility_v1_validation",
        "decision": decision,
