...
def shape_cycle(period, family, p1, p2=0.):
    ...
    if family == 'pulse':
        ...
        pulse = np.maximum(0, np.minimum(local / peak, (1 - local) / (1 - peak)))
        return .002 * pulse / pulse.max()
    if family == 'phase':
...
def cycle_panel(frame):
    ...
    for a, c in zip(lows[:-1], lows[1:]):
        ...
        if shape is None:
            rejected.append({'a': int(a), 'c': int(c), 'net_log': float(frame.pnl_log.iloc[a + 2:c + 2].sum())})
            continue
        assert not covered[a + 2:c + 2].any()
        covered[a + 2:c + 2] = True
        net = float(frame.pnl_log.iloc[a + 2:c + 2].sum())
        road = float(np.abs(frame.open_log_return.iloc[a + 2:c + 2]).sum())
        if road <= 0:
            raise ValueError('zero-price-path cycle')
        rows.append({'a': int(a), 'b': int(b), 'c': int(c),
            'pnl_first_index': int(a + 2), 'pnl_last_index': int(c + 1),
